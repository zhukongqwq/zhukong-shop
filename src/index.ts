...
// 辅助函数：获取用户积分
async function getUserBalance(ctx: Context, platform: string, userId: string): Promise<number> {
  try {
    const balance = await ctx.emit('currency/get', `${platform}:${userId}`)
...
// 辅助函数：扣除用户积分
async function deductUserBalance(ctx: Context, platform: string, userId: string, amount: number): Promise<boolean> {
  const balance = await getUserBalance(ctx, platform, userId)
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.command(`${config.commandPrefix}buy <itemName>`, `购买商店商品`)
    ...
    .action(async ({ session }, itemName) => {
      ...
      // 扣除积分
      const success = await deductUserBalance(ctx, platform!, userId!, item.price)
      if (!success) {
        return '购买失败，请稍后重试。'
      }
      ...
      if (config.enableLogging) {
        ctx.logger.info(`用户 ${platform}:${userId} 购买商品 ${item.name} (${item.price}积分)`)
      }
  ...
  ctx.command(`${config.commandPrefix}use <commandName>`, `使用已购买的命令`)
    ...
    .action(async ({ session }, commandName) => {
      ...
      if (usages.length === 0) {
        return config.messages.usageNoPermission
      }
      ...
      if (usage.remaining_uses <= 0) {
        return config.messages.usageExhausted
      }
      ...
      if (item.cooldown && item.cooldown > 0) {
        ...
        if (now.getTime() - lastUsed.getTime() < cooldownMs) {
          ...
          return formatMessage(config.messages.usageCooldown, { remainingTime }, config)
        }
      }
      ...
      if (config.enableLogging) {
        ctx.logger.info(`用户 ${platform}:${userId} 使用命令 ${commandName}，剩余次数: ${newRemaining}`)
      }
      
      return formatMessage(config.messages.usageSuccess, {
  ...
  ctx.command(`${config.commandPrefix}myitems`, `查看已购买的商品`)
    ...
    .action(async ({ session }) => {
      ...
      let message = config.messages.usageInfo + '\n'
      message += '='.repeat(20) + '\n'
      ...
      for (const purchase of purchases) {
        ...
        message += `   购买时间: ${purchase.purchased_at.toLocaleDateString()}\n`
        message += `   使用情况: ${remaining}/${total}次\n`
        
        if (item.cooldown) {
          message += `   冷却时间: ${item.cooldown}分钟\n`
        }
        message += '\n'
      }
  ...
  admin.subcommand('.update <itemId:number>', '更新商品信息')
    ...
    .action(async ({ session, options = {} }, itemId) => {
      ...
      const item = items[0]
      if (config.enableLogging) {
        ctx.logger.info(`管理员更新商品: ${item.name}`)
      }
  ...
  admin.subcommand('.delete <itemId:number>', '删除商品')
    .action(async ({ session }, itemId) => {
      ...
      if (config.enableLogging) {
        ctx.logger.info(`管理员删除商品: ${item.name}`)
      }
  ...
  admin.subcommand('.usage.add <target> <command> <amount:number>', '为用户增加命令使用次数')
    .action(async ({ session }, target, command, amount) => {
      ...
      if (config.enableLogging) {
        ctx.logger.info(`管理员为 ${platform}:${userId} 增加 ${command} 使用次数 ${amount} 次`)
      }
  ...
  if (config.enableAutoRestore) {
    ctx.on('ready', async () => {
      ctx.logger.info('开始初始化商店插件...')
      
      // 检查并创建必要的命令类型商品示例
      const existingItems = await ctx.database.get('shop_items', {})
      if (existingItems.length === 0 && config.enableLogging) {
        ctx.logger.info('商店为空，管理员可以使用 $shopadmin.add 添加商品')
      }
      
      ctx.logger.info('商店插件初始化完成')
    })
  }
  ...
  if (config.enableLogging) {
    ctx.middleware(async (session, next) => {
      const { content, platform, userId } = session
      ...
      if (content && content.startsWith(config.commandPrefix)) {
        ...
        if (items.length > 0) {
          ...
          if (usages.length === 0) {
            // 用户没有购买此命令
            return `你需要购买【${item.name}】才能使用 ${config.commandPrefix}${commandName} 命令\n` +
                   `使用 ${config.commandPrefix}shop 查看商店，${config.commandPrefix}buy ${item.name} 购买`
          }
          ...
          if (config.enableLogging) {
            ctx.logger.info(`用户 ${platform}:${userId} 通过商店使用命令 ${commandName}`)
          }
  ...
  ctx.on('ready', () => {
    ...
    ctx.logger.info(`使用 ${config.commandPrefix}shop 查看商店`)
    ctx.logger.info(`使用 ${config.commandPrefix}buy <商品名> 购买商品`)
    ctx.logger.info(`使用 ${config.commandPrefix}myitems 查看已购商品`)
    ctx.logger.info(`管理员使用 ${config.commandPrefix}shopadmin 管理商店`)
  })
}